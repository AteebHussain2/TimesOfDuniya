...
import { TRIGGER, TYPE } from "@prisma/client";

export default async function PendingJobsPage() {
  const pendingJobs = await GetPendingJobs(TRIGGER.CRON);
  ...
  return (
    <div className="space-y-6">
      ...
      {pendingJobs.length > 0 ? (
        pendingJobs.map((job) => <JobCard key={job.id} job={job} showGenerateButton={true && job.type !== TYPE.ARTICLE_GENERATION} />)
      ) : (
